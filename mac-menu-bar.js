// macOS Menu Bar JavaScript
class MacOSMenuBar {
  constructor(options = {}) {
    this.appName = options.appName || 'Calovetor';
    this.onMenuAction = options.onMenuAction || (() => {});
    this.activeMenu = null;
    this.dropdownPosition = { x: 0, y: 0 };
    this.submenuHideTimeout = null;
    
    this.init();
  }

  init() {
    this.createMenuBar();
    this.setupEventListeners();
    this.updateClock();
    this.startClock();
  }

  createMenuBar() {
    // Load the menu bar HTML
    fetch('mac-menu-bar.html')
      .then(response => response.text())
      .then(html => {
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = html;
        const menuBar = tempDiv.firstElementChild;
        
        // Insert at the beginning of body
        document.body.insertBefore(menuBar, document.body.firstChild);
        
        // Set app name
        document.getElementById('app-name').textContent = this.appName;
        
        // Setup menu data
        this.setupMenuData();
        
        // Setup event listeners after DOM is ready
        setTimeout(() => this.setupEventListeners(), 100);
      })
      .catch(error => {
        console.error('Error loading menu bar:', error);
        // Fallback: create menu bar directly
        this.createMenuBarDirectly();
      });
  }

  createMenuBarDirectly() {
    // Fallback method if fetch fails
    const menuBarHTML = `
      <div id="mac-menu-bar" class="mac-menu-bar">
        <div class="menu-bar-content">
          <div class="menu-bar-left">
            <div class="app-logo" id="apple-logo">
              <img src="E:/Hacking/testing/calculator/icons/app_icon.png" alt="App Icon" style="width: 15px; height: 15px; border-radius: 3px;">
            </div>
            <span class="app-name" id="app-name">${this.appName}</span>
            <div class="menu-items" id="menu-items">
              <span class="menu-item" data-menu="File">File</span>
              <span class="menu-item" data-menu="Edit">Edit</span>
              <span class="menu-item" data-menu="View">View</span>
              <span class="menu-item" data-menu="History">History</span>
              <span class="menu-item" data-menu="Window">Window</span>
              <span class="menu-item" data-menu="Developer">Developer</span>
              <span class="menu-item" data-menu="Help">Help</span>
            </div>
          </div>
          <div class="menu-bar-right">
            <div class="status-icon battery-icon">
              <svg width="26" height="12" viewBox="0 0 190 90" fill="none">
                <path d="M0 0 C0.82175217 -0.00511093 1.64350433 -0.01022186 2.49015808 -0.01548767 C5.19859152 -0.0293548 7.90676972 -0.02842931 10.61523438 -0.02612305 C12.50183201 -0.03001108 14.38842878 -0.03434597 16.27502441 -0.03910828 C20.22585897 -0.04656393 24.17660415 -0.04628359 28.12744141 -0.04101562 C33.18827521 -0.0352167 38.24874354 -0.05216538 43.30951691 -0.0753603 C47.20430639 -0.09009567 51.09900777 -0.09096969 54.99382019 -0.08785057 C56.85972315 -0.08836371 58.72563106 -0.09360009 60.59150696 -0.10366249 C63.20056908 -0.11600809 65.80891792 -0.10924706 68.41796875 -0.09765625 C69.18694992 -0.10529999 69.95593109 -0.11294373 70.74821472 -0.12081909 C76.48807519 -0.06449799 79.68979749 1.25547132 83.70898438 5.2746582 C85.12279298 9.05552515 84.88869294 13.09189823 84.91210938 17.07543945 C84.91780945 17.83003952 84.92350952 18.58463959 84.92938232 19.36210632 C84.93881056 20.95655932 84.94535764 22.55103161 84.94921875 24.14550781 C84.95890962 26.56846642 84.98994637 28.99057084 85.02148438 31.41333008 C85.02801682 32.96736153 85.03326635 34.52139902 85.03710938 36.07543945 C85.04945618 36.79234451 85.06180298 37.50924957 85.07452393 38.24787903 C85.05202575 43.40022518 84.24732046 46.86576899 81.70898438 51.2746582 C77.44198652 53.68062086 73.09614556 53.56449915 68.32617188 53.54931641 C67.49466614 53.55442734 66.6631604 53.55953827 65.80645752 53.56480408 C63.06555937 53.57867253 60.32491338 53.57774562 57.58398438 53.57543945 C55.6749665 53.57932741 53.76594949 53.58366229 51.85693359 53.58842468 C47.85902093 53.59588062 43.86119658 53.59559979 39.86328125 53.59033203 C34.74191497 53.58453311 29.62090983 53.60148166 24.49960327 53.6246767 C20.5584909 53.63941144 16.61746561 53.64028618 12.67633057 53.63716698 C10.788099 53.63768015 8.89986255 53.64291715 7.01165771 53.6529789 C4.37146537 53.66532324 1.73197803 53.65856418 -0.90820312 53.64697266 C-2.07570099 53.65843826 -2.07570099 53.65843826 -3.26678467 53.6701355 C-7.79019075 53.62626694 -10.68014088 53.04878224 -14.29101562 50.2746582 C-17.11730625 46.65525677 -16.59684928 42.17147887 -16.59179688 37.7824707 C-16.596082 36.95891083 -16.60036713 36.13535095 -16.6047821 35.28683472 C-16.61051687 33.54694537 -16.61103195 31.80703205 -16.60668945 30.06713867 C-16.60353488 27.4158396 -16.62697968 24.76566273 -16.65234375 22.11450195 C-16.65461749 20.42049294 -16.65531989 18.72648094 -16.65429688 17.0324707 C-16.66350662 16.24478302 -16.67271637 15.45709534 -16.6822052 14.64553833 C-16.64212062 9.83558309 -16.12088551 6.35019582 -13.29101562 2.2746582 C-9.00849445 -0.06521491 -4.75970999 -0.0153343 0 0 Z " fill="#FFFFFF" transform="translate(31.291015625,14.725341796875)"/>
                <path d="M0 0 C1.03584732 -0.00571014 2.07169464 -0.01142029 3.13893127 -0.01730347 C4.26990234 -0.01842133 5.40087341 -0.01953918 6.56611633 -0.02069092 C8.35947166 -0.02813828 8.35947166 -0.02813828 10.1890564 -0.03573608 C13.4661846 -0.04783365 16.74327716 -0.05530034 20.02042365 -0.05974674 C22.07083709 -0.06268252 24.12124501 -0.06678835 26.17165565 -0.07125092 C32.59544399 -0.08492098 39.01921961 -0.09458922 45.44302148 -0.09845281 C52.84210819 -0.10292813 60.24100968 -0.1204535 67.6400401 -0.1494534 C73.36930485 -0.17112394 79.09851771 -0.18115999 84.82782298 -0.18249393 C88.24476141 -0.18353841 91.6615256 -0.18936922 95.07842064 -0.20731354 C98.89607549 -0.22441187 102.71328223 -0.22248399 106.53096008 -0.21600342 C107.65181 -0.2252182 108.77265991 -0.23443298 109.92747498 -0.243927 C117.95269142 -0.20135944 124.42140542 0.86173116 131.18452454 5.32794189 C136.30866063 10.6904099 138.96352158 16.97584486 139.03315735 24.39898682 C139.04276489 25.2305278 139.05237244 26.06206879 139.06227112 26.91880798 C139.06864594 28.2563488 139.06864594 28.2563488 139.07514954 29.62091064 C139.08084961 30.54833572 139.08654968 31.4757608 139.09242249 32.43128967 C139.10186112 34.39398055 139.10840195 36.35668719 139.11225891 38.31939697 C139.12193301 41.2997461 139.15295424 44.27939605 139.18452454 47.25958252 C139.19105839 49.17168782 139.19630736 51.08379804 139.20014954 52.99591064 C139.21249634 53.87748337 139.22484314 54.75905609 139.23756409 55.66734314 C139.21227077 62.80842015 138.04416634 69.6473711 133.62202454 75.45294189 C121.3613082 87.43500559 100.42474177 84.44038848 84.55952454 84.40606689 C82.33385679 84.40741811 80.10818933 84.40936241 77.88252258 84.41186523 C73.24325214 84.41479505 68.60405931 84.41058841 63.96479797 84.40118408 C58.06132833 84.38979096 52.15804063 84.396362 46.25457764 84.40832901 C41.6662411 84.41560584 37.07794667 84.41323313 32.48960876 84.40807343 C30.31462074 84.40675298 28.13962872 84.40832734 25.96464539 84.41303253 C-10.09097031 84.47394262 -10.09097031 84.47394262 -18.12797546 77.14044189 C-23.90359816 71.3648192 -25.45954292 64.46402857 -25.54481506 56.57830811 C-25.53874237 55.66037476 -25.53266968 54.74244141 -25.52641296 53.79669189 C-25.52928314 52.82816284 -25.53215332 51.85963379 -25.53511047 50.86175537 C-25.5371418 48.82459932 -25.5316775 46.78742334 -25.51908875 44.75030518 C-25.50302206 41.64943422 -25.51896955 38.54986653 -25.53813171 35.44903564 C-25.53614912 33.46075155 -25.5323051 31.47246825 -25.52641296 29.48419189 C-25.53248566 28.56625854 -25.53855835 27.6483252 -25.54481506 26.70257568 C-25.46603492 19.41721627 -24.07647694 14.03535838 -19.94047546 7.89044189 C-14.28843724 2.36400452 -7.83600922 0.02592702 0 0 Z M-12.12797546 14.82794189 C-16.85057138 22.64974138 -16.47096343 31.04435231 -16.44364929 39.92144775 C-16.4404874 42.06983139 -16.46401315 44.21683688 -16.48930359 46.36505127 C-16.72736225 58.22940199 -16.72736225 58.22940199 -11.85282898 68.71026611 C-5.34263349 74.74736513 2.67544426 74.43516039 10.98872375 74.4151001 C12.08719635 74.42021103 13.18566895 74.42532196 14.31742859 74.43058777 C17.93724565 74.44445184 21.55687163 74.44352931 25.17671204 74.44122314 C27.70105561 74.44511188 30.22539853 74.44944692 32.7497406 74.45420837 C38.03710827 74.4616638 43.32440627 74.46152321 48.61177063 74.45611572 C54.70516317 74.45007927 60.79832461 74.46238391 66.89167774 74.48368579 C72.77512247 74.50349016 78.65847729 74.5064679 84.54195023 74.50295067 C87.03533089 74.50346296 89.52871531 74.50868326 92.02207565 74.51876259 C95.51374184 74.5311454 99.00487808 74.52432362 102.49653625 74.51275635 C103.51916565 74.52040009 104.54179504 74.52804382 105.59541321 74.53591919 C113.22652315 74.48024712 120.0214064 73.74845424 125.80952454 68.45294189 C130.56241065 60.81951874 130.21555801 52.56684234 130.18769836 43.84161377 C130.18454163 41.77662136 130.20804614 39.71307024 130.23335266 37.64825439 C130.40830464 25.44302856 130.40830464 25.44302856 125.63227844 14.5022583 C119.1868619 8.43943647 110.09608162 8.84539037 101.84126282 8.86578369 C100.76375763 8.86067276 99.68625244 8.85556183 98.57609558 8.85029602 C95.03145213 8.83645148 91.48700424 8.83735298 87.94233704 8.83966064 C85.46665882 8.83577087 82.99098126 8.83143562 80.51530457 8.82667542 C75.33226862 8.81922422 70.14930081 8.81949652 64.96626282 8.82476807 C58.34000023 8.83056079 51.71401734 8.81363077 45.08780098 8.79042339 C39.97640872 8.77565762 34.86508337 8.77481825 29.75367355 8.77793312 C27.31135886 8.77742133 24.86904028 8.77221043 22.42674637 8.7621212 C19.0048403 8.74971941 15.58347342 8.75657139 12.16157532 8.76812744 C11.16281372 8.7604837 10.16405212 8.75283997 9.13502502 8.7449646 C1.89083821 8.7987594 -6.88444207 9.18240837 -12.12797546 14.82794189 Z " fill="#FFFFFF" transform="translate(25.127975463867188,-0.14044189453125)"/>
                <path d="M0 0 C6.61183005 2.20394335 7.27129348 2.77981088 11 8 C12.63537738 12.90613215 12.50113348 17.88031198 12 23 C10.1875 26.5625 10.1875 26.5625 8 29 C7.2884375 29.8971875 7.2884375 29.8971875 6.5625 30.8125 C5 32 5 32 0 33 C0 22.11 0 11.22 0 0 Z " fill="#FFFFFF" transform="translate(171,25)"/>
              </svg>
            </div>
            <div class="status-icon wifi-icon">
              <svg width="20" height="14" viewBox="0 0 150 110" fill="none">
                <path d="M0 0 C0.65500488 0.20898926 1.31000977 0.41797852 1.98486328 0.63330078 C14.82324128 4.91062412 25.79730909 11.812883 35.8125 20.8125 C36.57759888 21.49638794 36.57759888 21.49638794 37.3581543 22.1940918 C39.29498348 24.01298915 40.84104556 25.67845094 42.0234375 28.0703125 C41.98868831 30.93132924 40.7368247 32.00327419 38.8125 34.0625 C38.20019531 34.73410156 37.58789062 35.40570313 36.95703125 36.09765625 C33.07626002 39.86998278 33.07626002 39.86998278 31 41 C26.85231569 40.06955581 24.51783886 37.81497315 21.5625 34.875 C4.56703863 19.28329355 -16.81223148 15.07788117 -39.23388672 15.82958984 C-58.92928161 17.04295133 -76.63301992 28.19464819 -91 41 C-96.17940014 39.58743632 -99.965861 36.34834456 -103 32 C-103.39867702 28.27901445 -102.98432718 26.38320487 -100.8046875 23.3203125 C-76.41780756 -3.27241587 -33.31208663 -10.63151701 0 0 Z" fill="#FFFFFF" transform="translate(103,4)"/>
                <path d="M0 0 C1.8984375 2.05078125 1.8984375 2.05078125 2.8984375 5.05078125 C0.89685511 7.9957076 -1.65789629 10.46703226 -4.1015625 13.05078125 C-5.1534375 14.31921875 -5.1534375 14.31921875 -6.2265625 15.61328125 C-8.1015625 17.05078125 -8.1015625 17.05078125 -10.4609375 16.84375 C-13.41993192 15.95517623 -14.91300535 14.82404416 -17.2265625 12.80078125 C-27.98918153 4.03925219 -39.5915468 1.83255625 -53.1015625 3.05078125 C-61.85246663 4.93211813 -69.21440161 9.60534319 -75.8515625 15.48828125 C-78.1015625 17.05078125 -78.1015625 17.05078125 -80.44140625 16.9140625 C-83.77758701 15.83139591 -85.4436323 14.27530835 -87.9140625 11.80078125 C-88.70425781 11.0325 -89.49445313 10.26421875 -90.30859375 9.47265625 C-92.1015625 7.05078125 -92.1015625 7.05078125 -92.62109375 4.52734375 C-91.49952622 -0.81907591 -86.29726382 -3.87482782 -82.1015625 -6.94921875 C-74.21821198 -11.66652861 -66.12451308 -15.1700454 -57.1015625 -16.94921875 C-56.39128906 -17.09101562 -55.68101563 -17.2328125 -54.94921875 -17.37890625 C-34.50294745 -20.50105568 -14.99849748 -13.69736599 0 0 Z" fill="#FFFFFF" transform="translate(117.1015625,53.94921875)"/>
                <path d="M0 0 C2.55892967 1.51276457 3.9662918 2.74643145 6 5 C6.5234375 7.25390625 6.5234375 7.25390625 6 10 C4.23657597 12.43682265 2.11984755 14.43955796 -0.0625 16.5 C-0.62388672 17.05558594 -1.18527344 17.61117187 -1.76367188 18.18359375 C-5.58232446 21.89314197 -9.39060132 25.31429256 -14 28 C-19.80211565 26.44161162 -23.60213327 22.18619112 -27.875 18.1875 C-28.67164063 17.47529297 -29.46828125 16.76308594 -30.2890625 16.02929688 C-31.03671875 15.33255859 -31.784375 14.63582031 -32.5546875 13.91796875 C-33.24030762 13.28656982 -33.92592773 12.6551709 -34.63232422 12.00463867 C-36 10 -36 10 -35.83251953 7.4152832 C-34.58453344 3.79465962 -33.03776765 2.36945876 -30 0 C-18.999621 -5.30221711 -11.00576064 -5.15947378 0 0 Z" fill="#FFFFFF" transform="translate(87,77)"/>
              </svg>
            </div>
            <span class="clock" id="menu-clock">12:00 AM</span>
          </div>
        </div>
        <div id="menu-dropdown" class="menu-dropdown" style="display: none;">
          <div class="dropdown-content" id="dropdown-content"></div>
        </div>
      </div>
    `;
    
    document.body.insertAdjacentHTML('afterbegin', menuBarHTML);
    this.setupMenuData();
    setTimeout(() => this.setupEventListeners(), 100);
  }

  setupMenuData() {
    this.menuData = {
      apple: [
        { label: 'About Calovetor', action: 'about' },
        { type: 'separator' },
        { label: 'Preferences...', action: 'settings', shortcut: '⌘,' },
        { type: 'separator' },
        { label: 'Hide Calovetor', action: 'minimize', shortcut: '⌘M' },
        { label: 'Hide Others', action: 'hide-others', shortcut: '⌥⌘H' },
        { label: 'Show All', action: 'show-all', shortcut: '⌥⌘A' },
        { type: 'separator' },
        { label: 'Quit Calovetor', action: 'close-window', shortcut: '⌘Q' }
      ],
      File: [
        { label: 'New Calculation', action: 'clear', shortcut: '⌘N' },
        { type: 'separator' },
        { label: 'Save Calculation', action: 'save-page', shortcut: '⌘S' },
        { label: 'Take Snapshot', action: 'take-snapshot', shortcut: '⇧⌘S' },
        { type: 'separator' },
        { label: 'Close App', action: 'close-window', shortcut: '⌘Q' }
      ],
      Edit: [
        { label: 'Undo', action: 'undo', shortcut: '⌘Z' },
        { label: 'Redo', action: 'redo', shortcut: '⇧⌘Z' },
        { type: 'separator' },
        { label: 'Copy All Values', action: 'copy-all', shortcut: '⌘C' },
        { label: 'Copy Answer', action: 'copy-answer', shortcut: '⇧⌘C' },
        { label: 'Paste All Values', action: 'paste-all', shortcut: '⌘V' },
        { label: 'Paste Answer', action: 'paste-answer', shortcut: '⇧⌘V' },
        { label: 'Select All', action: 'select-all', shortcut: '⌘A' },
        { type: 'separator' },
        { label: 'More', action: 'more', hasSubmenu: true }
      ],
      View: [
        { label: 'Zoom In', action: 'zoom-in', shortcut: '⌘+' },
        { label: 'Zoom Out', action: 'zoom-out', shortcut: '⌘-' },
        { label: 'Actual Size', action: 'zoom-reset', shortcut: '⌘0' },
        { label: 'Fullscreen', action: 'fullscreen', shortcut: '⌃⌘F' },
        { type: 'separator' },
        { label: 'Themes', action: 'themes', hasSubmenu: true },
        { type: 'separator' },
        { label: 'Dark Mode', action: 'toggle-dark', shortcut: '⌘D' },
        { label: 'Pookie Mode', action: 'toggle-pookie', shortcut: '⌘P' }
      ],
      History: [
        { label: 'Open History', action: 'history', shortcut: '⌘H' },
        { label: 'Clear History', action: 'clear-history', shortcut: '⇧⌘C' },
        { type: 'separator' },
        { label: 'Save Calculation', action: 'save-page', shortcut: '⌘S' },
        { label: 'Export History', action: 'export-history', shortcut: '⇧⌘E' },
        { type: 'separator' },
        { label: 'Recent Calculations', action: 'recent-calculations', hasSubmenu: true }
      ],
      Window: [
        { label: 'History', action: 'window-history', hasSubmenu: true },
        { label: 'Config', action: 'window-config', hasSubmenu: true },
        { label: 'Aesthetics', action: 'window-aesthetics', hasSubmenu: true },
        { label: 'Sounds', action: 'window-sounds', hasSubmenu: true },
        { label: 'Keys', action: 'window-keys', hasSubmenu: true },
        { label: 'Developer', action: 'window-developer', hasSubmenu: true }
      ],
      Developer: [
        { label: 'Developer Info', action: 'about' },
        { label: 'Socials', action: 'socials' },
        { label: 'Portfolio', action: 'portfolio' },
        { label: 'Resume', action: 'resume' },
        { label: 'See More...', action: 'see-more-dev' }
      ],
      Help: [
        { label: 'Keyboard Shortcuts', action: 'shortcuts', shortcut: '⌘?' },
        { type: 'separator' },
        { label: 'Report an Issue', action: 'contact-support' },
        { label: 'Check for Updates', action: 'check-updates' }
      ]
    };

    // Submenu data
    this.submenuData = {
      themes: [
        { label: 'Alchemical Reaction', action: 'theme-alchemical' },
        { label: 'Autumn Leaves', action: 'theme-autumn' },
        { label: 'Autumn Orchard', action: 'theme-autumn-orchard' },
        { label: 'Beachfront Views', action: 'theme-beachfront' },
        { label: 'Blooming Romance', action: 'theme-blooming' },
        { label: 'Blue Eclipse', action: 'theme-blue-eclipse' },
        { label: 'Burgundy Elegance', action: 'theme-burgundy' },
        { label: 'Cherry Blossom', action: 'theme-cherry' },
        { label: 'Cobalt Dreams', action: 'theme-cobalt' },
        { label: 'Coral Sunset', action: 'theme-coral' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-themes' }
      ],
      'recent-calculations': [
        { label: '2 + 2 = 4', action: 'recent-1' },
        { label: '10 × 5 = 50', action: 'recent-2' },
        { label: 'π × 3.14 = 9.86', action: 'recent-3' },
        { label: '√144 = 12', action: 'recent-4' },
        { label: '2^8 = 256', action: 'recent-5' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-recent' }
      ],
      'window-history': [
        { label: 'All', action: 'history-all' },
        { label: 'Recents', action: 'history-recents' },
        { label: 'Sort', action: 'history-sort' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-history' }
      ],
      'window-config': [
        { label: 'Input', action: 'config-input' }
      ],
      'window-aesthetics': [
        { label: 'Themes', action: 'aesthetic-themes' },
        { label: 'Layout', action: 'aesthetic-layout' },
        { label: 'Colors', action: 'aesthetic-colors' },
        { label: 'Buttons', action: 'aesthetic-buttons' },
        { label: 'Typography', action: 'aesthetic-typography' },
        { label: 'Effects', action: 'aesthetic-effects' },
        { label: 'Advances', action: 'aesthetic-advances' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-aesthetics' }
      ],
      'window-sounds': [
        { label: 'Sounds', action: 'sounds-main' },
        { label: 'Gestures', action: 'sounds-gestures' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-sounds' }
      ],
      'window-keys': [
        { label: 'Calculator', action: 'keys-calculator' },
        { label: 'System', action: 'keys-system' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-keys' }
      ],
      'window-developer': [
        { label: 'Developer', action: 'developer-main' },
        { label: 'Socials', action: 'developer-socials' },
        { label: 'Portfolio', action: 'developer-portfolio' },
        { label: 'Resume', action: 'developer-resume' },
        { type: 'separator' },
        { label: 'See More...', action: 'see-more-developer' }
      ]
    };
  }

  setupEventListeners() {
    // Apple logo click
    const appleLogo = document.getElementById('apple-logo');
    if (appleLogo) {
      appleLogo.addEventListener('click', (e) => {
        e.stopPropagation();
        this.handleAppleMenuClick();
      });
    }

    // Menu item clicks
    const menuItems = document.querySelectorAll('.menu-item');
    menuItems.forEach(item => {
      item.addEventListener('click', (e) => {
        e.stopPropagation();
        const menuName = item.dataset.menu;
        this.handleMenuItemClick(menuName);
      });
    });

    // Click outside to close dropdown
    document.addEventListener('click', (e) => {
      const menuBar = document.getElementById('mac-menu-bar');
      const dropdown = document.getElementById('menu-dropdown');
      
      if (menuBar && !menuBar.contains(e.target) && dropdown && !dropdown.contains(e.target)) {
        this.closeDropdown();
      }
    });

    // Keyboard shortcuts
    document.addEventListener('keydown', (e) => {
      this.handleKeyboardShortcuts(e);
    });
  }

  handleAppleMenuClick() {
    if (this.activeMenu === 'apple') {
      this.closeDropdown();
    } else {
      const appleLogo = document.getElementById('apple-logo');
      if (appleLogo) {
        const rect = appleLogo.getBoundingClientRect();
        const parentRect = appleLogo.offsetParent?.getBoundingClientRect() || { left: 0, top: 0 };
        this.showDropdown('apple', rect.left - parentRect.left, 34);
      }
    }
  }

  handleMenuItemClick(menuName) {
    // Always close existing dropdown first
    this.closeDropdown();
    
    // Then show the new dropdown
    const menuItem = document.querySelector(`[data-menu="${menuName}"]`);
    if (menuItem) {
      const rect = menuItem.getBoundingClientRect();
      const parentRect = menuItem.offsetParent?.getBoundingClientRect() || { left: 0, top: 0 };
      this.showDropdown(menuName, rect.left - parentRect.left, 34);
    }
  }

  showDropdown(menuName, x, y) {
    let dropdown = document.getElementById('menu-dropdown');
    let dropdownContent;
    
    // Create dropdown if it doesn't exist
    if (!dropdown) {
      dropdown = document.createElement('div');
      dropdown.id = 'menu-dropdown';
      dropdown.className = 'menu-dropdown';
      
      dropdownContent = document.createElement('div');
      dropdownContent.className = 'dropdown-content';
      dropdown.appendChild(dropdownContent);
      
      document.getElementById('mac-menu-bar').appendChild(dropdown);
    } else {
      dropdownContent = dropdown.querySelector('.dropdown-content');
    }
    
    const items = this.menuData[menuName] || this.menuData.apple;
    
    // Clear existing content
    dropdownContent.innerHTML = '';
    
    // Add menu items
    items.forEach(item => {
      if (item.type === 'separator') {
        const separator = document.createElement('div');
        separator.className = 'dropdown-separator';
        dropdownContent.appendChild(separator);
      } else {
        const menuItem = document.createElement('div');
        menuItem.className = 'dropdown-item';
        if (item.hasSubmenu) {
          menuItem.className += ' has-submenu';
        }
        
        const label = document.createElement('span');
        label.textContent = item.label;
        menuItem.appendChild(label);
        
        if (item.shortcut) {
          const shortcut = document.createElement('span');
          shortcut.className = 'dropdown-shortcut';
          shortcut.textContent = item.shortcut;
          menuItem.appendChild(shortcut);
        }
        
        if (item.hasSubmenu) {
          // Add submenu hover functionality with delay
          let submenuTimeout;
          
          menuItem.addEventListener('mouseenter', (e) => {
            clearTimeout(this.submenuHideTimeout);
            clearTimeout(submenuTimeout);
            submenuTimeout = setTimeout(() => {
              this.showSubmenu(item.action, e.target);
            }, 100); // Small delay to prevent accidental triggering
          });
          
          menuItem.addEventListener('mouseleave', () => {
            clearTimeout(submenuTimeout);
            // Don't hide immediately, give user time to move to submenu
            this.submenuHideTimeout = setTimeout(() => {
              this.hideSubmenu();
            }, 300);
          });
          
          // Add chevron icon for submenu items
          const chevron = document.createElement('span');
          chevron.className = 'dropdown-chevron';
          chevron.textContent = '›';
          chevron.style.marginLeft = 'auto';
          chevron.style.opacity = '0.7';
          chevron.style.fontSize = '16px';
          chevron.style.lineHeight = '1';
          menuItem.appendChild(chevron);
        } else {
          menuItem.addEventListener('click', (e) => {
            e.stopPropagation();
            if (item.action) {
              this.onMenuAction(item.action);
            }
            this.closeDropdown();
          });
        }
        
        dropdownContent.appendChild(menuItem);
      }
    });
    
    // Position and show dropdown
    dropdown.style.left = `${x}px`;
    dropdown.style.top = `${y}px`;
    dropdown.style.display = 'block';
    dropdown.style.opacity = '1';
    dropdown.style.visibility = 'visible';
    this.activeMenu = menuName;
  }

  showSubmenu(submenuAction, parentItem) {
    // Mark existing submenu as old for fade effect
    const existingSubmenu = document.getElementById('active-submenu');
    if (existingSubmenu) {
      existingSubmenu.classList.add('old');
      existingSubmenu.id = ''; // Remove ID so new submenu can take it
      
      // Remove old submenu after animation completes
      setTimeout(() => {
        if (existingSubmenu.parentNode) {
          existingSubmenu.remove();
        }
      }, 200); // Match the CSS animation duration
    }
    
    const submenuItems = this.submenuData[submenuAction];
    if (!submenuItems) return;
    
    // Create new submenu dropdown
    const submenu = document.createElement('div');
    submenu.className = 'submenu-dropdown';
    submenu.id = 'active-submenu';
    
    const submenuContent = document.createElement('div');
    submenuContent.className = 'dropdown-content';
    
    // Add submenu items
    submenuItems.forEach(item => {
      if (item.type === 'separator') {
        const separator = document.createElement('div');
        separator.className = 'dropdown-separator';
        submenuContent.appendChild(separator);
      } else {
        const menuItem = document.createElement('div');
        menuItem.className = 'dropdown-item';
        
        const label = document.createElement('span');
        label.textContent = item.label;
        menuItem.appendChild(label);
        
        menuItem.addEventListener('click', (e) => {
          e.stopPropagation();
          if (item.action) {
            this.onMenuAction(item.action);
          }
          this.closeDropdown();
        });
        
        submenuContent.appendChild(menuItem);
      }
    });
    
    submenu.appendChild(submenuContent);
    
    // Position submenu to the right of parent item
    const parentRect = parentItem.getBoundingClientRect();
    const menuBar = document.getElementById('mac-menu-bar');
    const menuBarRect = menuBar.getBoundingClientRect();
    
    submenu.style.position = 'fixed';
    submenu.style.left = `${parentRect.right - menuBarRect.left + 5}px`;
    submenu.style.top = `${parentRect.top - menuBarRect.top}px`;
    submenu.style.display = 'block';
    
    // Add hover listeners to keep submenu open
    submenu.addEventListener('mouseenter', () => {
      clearTimeout(this.submenuHideTimeout);
    });
    
    submenu.addEventListener('mouseleave', () => {
      this.submenuHideTimeout = setTimeout(() => {
        this.hideSubmenu();
      }, 200); // Reduced from 300ms
    });
    
    menuBar.appendChild(submenu);
  }

  hideSubmenu() {
    const existingSubmenu = document.getElementById('active-submenu');
    if (existingSubmenu) {
      existingSubmenu.classList.add('old');
      existingSubmenu.id = ''; // Remove ID
      
      // Remove after fade animation
      setTimeout(() => {
        if (existingSubmenu.parentNode) {
          existingSubmenu.remove();
        }
      }, 200);
    }
    clearTimeout(this.submenuHideTimeout);
  }

  closeDropdown() {
    const dropdown = document.getElementById('menu-dropdown');
    if (dropdown) {
      dropdown.classList.add('hiding');
      dropdown.id = ''; // Remove ID
      
      // Remove after fade animation
      setTimeout(() => {
        if (dropdown.parentNode) {
          dropdown.remove();
        }
      }, 150); // Match the CSS animation duration
    }
    this.hideSubmenu();
    this.activeMenu = null;
  }

  handleKeyboardShortcuts(e) {
    // Handle common macOS shortcuts
    const key = e.key;
    const cmd = e.metaKey || e.ctrlKey;
    const shift = e.shiftKey;
    const option = e.altKey;
    
    if (cmd) {
      switch (key) {
        case 'c':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('copy');
          }
          break;
        case 'v':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('paste');
          }
          break;
        case 'x':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('cut');
          }
          break;
        case 'a':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('select-all');
          }
          break;
        case 'z':
          if (!option) {
            e.preventDefault();
            this.onMenuAction(shift ? 'redo' : 'undo');
          }
          break;
        case 'f':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('find');
          }
          break;
        case 'p':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('print');
          }
          break;
        case 's':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('save-page');
          }
          break;
        case 'w':
          if (!option) {
            e.preventDefault();
            this.onMenuAction(shift ? 'close-window' : 'close-tab');
          }
          break;
        case 'n':
          if (!option) {
            e.preventDefault();
            this.onMenuAction(shift ? 'new-private' : 'new-window');
          }
          break;
        case 'o':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('open-file');
          }
          break;
        case 'm':
          if (!option && !shift) {
            e.preventDefault();
            this.onMenuAction('minimize');
          }
          break;
      }
    }
  }

  updateClock() {
    const clockElement = document.getElementById('menu-clock');
    if (clockElement) {
      const now = new Date();
      const timeString = now.toLocaleTimeString('en-US', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true
      });
      clockElement.textContent = timeString;
    }
  }

  startClock() {
    this.updateClock();
    setInterval(() => this.updateClock(), 60000); // Update every minute
  }

  // Public methods
  setAppName(name) {
    this.appName = name;
    const appNameElement = document.getElementById('app-name');
    if (appNameElement) {
      appNameElement.textContent = name;
    }
  }

  destroy() {
    const menuBar = document.getElementById('mac-menu-bar');
    if (menuBar) {
      menuBar.remove();
    }
  }
}

// Export for use in other scripts
window.MacOSMenuBar = MacOSMenuBar;
